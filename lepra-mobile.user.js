...
(function () {
  ...
  function tuneLoad() {
    var raw;
    try { raw = localStorage.getItem(TUNE_KEY); } catch (e) { return; }
    if (!raw) return;
    var data;
  ...
  function tuneSave() {
    ...
    try {
      if (Object.keys(out).length) localStorage.setItem(TUNE_KEY, JSON.stringify(out));
      else localStorage.removeItem(TUNE_KEY);
    } catch (e) { /* писать некуда — настройка проживёт до перезагрузки */ }
  }
...
`;
  ...
  (function migrateScKey() {
    try {
      var old = localStorage.getItem('lm-boom');
      if (old === null) return;
      if (localStorage.getItem(SC_KEY) === null) localStorage.setItem(SC_KEY, old);
      localStorage.removeItem('lm-boom');
    } catch (e) { /* приватный режим — переносить нечем, и не надо */ }
  })();
  ...
  function discoMode() {
    var v;
    try { v = localStorage.getItem(DISCO_KEY); } catch (e) { return 0; }
    var n = parseInt(v, 10);
    return (n === 1 || n === 2) ? n : 0;
  }

  function discoRemember(mode) {
    try { localStorage.setItem(DISCO_KEY, String(mode)); } catch (e) {}
  }
  ...
  function scWanted() {
    try { return localStorage.getItem(SC_KEY) === '1'; } catch (e) { return false; }
  }

  function scRemember(on) {
    try { localStorage.setItem(SC_KEY, on ? '1' : '0'); } catch (e) {}
  }
  ...
  function reportDisco(L) {
    ...
    if (discoThread()) {
      ...
      var all = document.querySelectorAll('#js-comments .comment').length;
