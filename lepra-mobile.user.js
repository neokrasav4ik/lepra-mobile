...
(function () {
  ...
  var CFG = {
    ...
    /* НАСТРОЙКА: свои значки вместо картинок лепры.
       logoVector — рисовать логотип (маску со звездой) фигурами вместо
       lepro_logo_mask.png. Растр у лепры 50 на 47, а в шапке стоит с
       потолком 40 — то есть почти без увеличения, и на экране с
       удвоенной плотностью точек он мылится. Вектор чётче на любой
       плотности и на любом масштабе страницы.
       false возвращает картинку: она никуда не девается, мы её только
       гасим правилом, поэтому откат мгновенный и без следов.
       crestVector — то же для герба «Блогов Империи» в строке
       навигационной штуки. Тут выигрыш не только в чёткости: адрес
       картинки берётся со страницы или из памяти, и на внутренних
       страницах, где правой колонки нет, гнездо оставалось пустым.
       Свой значок есть всегда.
    ...
    /* НАСТРОЙКА: прыгалки — навигация по комментариям справа по центру.
       ...
       чистый исходный вид.
       jumpHaloIn / jumpHaloOut — радиусы размытия ореола в пикселях
       при значке в jumpSize. Их два, потому что ореол складывается из
       двух теней: ближняя, почти непрозрачная, даёт кайму, дальняя —
       мягкое свечение вокруг. Оба растут вместе со значком (см.
       JUMP_HALO1 и JUMP_HALO2), поэтому числа заданы для заводского
       размера, а не для того, что окажется на экране. */
    jumpSize: 40,
    jumpRight: 10,
    jumpGap: 2,
    ...
    /* НАСТРОЙКА: парящие значки — поправка на масштаб страницы.
       Сорок пикселей CSS равны сорока пикселям экрана только при
       стопроцентном масштабе. Меню «Аа» в Safari масштаб меняет, и
       вместе с ним меняется палец: при 85% значок доходит до глаза
       тридцатью четырьмя, при увеличенном — раздувается. Кнопка,
       в которую целятся вслепую большим пальцем, обязана быть одного
       размера всегда, поэтому размер считается делением на
       visualViewport.scale — тем же приёмом и по той же причине, что
       кегль полей формы (см. CFG.formFont).
       Поправка одна на всех, кто висит поверх страницы: прыгалки
       справа, стрелка стыковки у левого края и гнездо у полосы
       прозрачности. Расходиться им нельзя — стыковка тем и делается,
       что стрелку подводят к гнезду, а расстояние между ними читается
       только вместе с их размером.
       jumpZoom — вести ли поправку вообще. false возвращает прежнее
       поведение: сорок пикселей CSS при любом масштабе.
       jumpMin / jumpMax — пределы в пикселях CSS. Нужны с обеих
       сторон: при масштабе 200% деление дало бы двадцать, при 50% —
       восемьдесят, и в обоих концах кнопка перестала бы быть кнопкой.
       Просвет между значками и отступ от края считаются НЕ делением, а
       от полученного размера — тем же множителем, что вышел у значка.
       Иначе при упоре в предел столбик разъезжался бы: значок стоит,
       просветы едут.
       Допуск стыковки (discoDock) поправке НЕ подчиняется: он задан
       ползунком в пикселях, и уводить его от показанного там числа
       значило бы врать в двух местах сразу. */
    jumpZoom: true,
    jumpMin: 24,
    jumpMax: 64,

    /* НАСТРОЙКА: собственный тон парящих значков.
       jumpColor выше — цвет лепры, светло-серый. На десктопе значки
       лежат в пустом поле справа, на телефоне — поверх текста поста, и
       светлое на светлом видно плохо. Берём тон сильно темнее и
       возвращаем читаемость того, что под значком, прозрачностью:
       сплошное тёмное пятно в сорок пикселей посреди строчек мешало бы
       не меньше.
       Тон общий на все три: прыгалки, стрелку стыковки и метку у
       полосы прозрачности. Все трое — значки одного рода и одной
       работы, и разъезжаться им незачем; своё есть только у гнезда в
       режиме стыковки, оно цель, а не указатель (см. discoNestInk).
       jumpColor при этом остался и не выброшен: из него по-прежнему
       ставится заливка-атрибут при сборке фигур — на случай, если
       правило почему-либо не доедет, значок будет виден.
       jumpInk — тон заливки от нуля (чёрный) до 255. Серый, одним
       числом на все три канала: цветных значков у лепры тут нет.
       jumpInkA — непрозрачность заливки, от 0 до 1.
       Глаза маски прозрачности не получают: они и так мелкие, а
       красная точка — единственное цветное пятно во всём наборе.
       В тёмной теме тон отражается (255 − jumpInk), чтобы значок был
       настолько же светлее чёрной страницы, насколько здесь темнее
       белой; непрозрачность остаётся та же. */
    jumpInk: 105,
    jumpInkA: 0.46,
    ...
    /* НАСТРОЙКА: гнездо под стрелку у полосы прозрачности.

       Один узел живёт в двух видах, и красятся они по-разному.

       БЕЗ scroll-connectа это просто стрелка вниз: по ней разворачивают
       пост тапом. Она — цель для пальца поверх текста, ровно как
       прыгалки справа, и красится теми же jumpInk / jumpInkA. Своих
       настроек у неё нет намеренно: два тона для двух одинаковых по
       смыслу значков разъехались бы при первой же подкрутке.

       СО scroll-connectом тот же узел становится гнездом: в него
       предстоит вводить стрелку прокруткой, и он обязан читаться целью,
       а не украшением. Поэтому тон тут свой, тёмный, и прозрачности
       заметно меньше, чем у прыгалок, — сквозь гнездо тоже должно быть
       что-то видно, но не настолько, чтобы оно потерялось в строчках.

       discoNestInk — тон гнезда в режиме стыковки, от 0 до 255.
       discoNestA — его непрозрачность.
       discoNest — если непусто, целиком заменяет собой оба числа
       (готовая строка цвета). Пустая строка — обычный случай.

       Раньше здесь вместо чисел стояло вычисление «обратный к
       jumpColor», и держалась на нём пара «светлая стрелка — тёмное
       гнездо». Вычисление убрано: прыгалки со стрелкой больше не
       красятся из jumpColor, и обращать стало нечего — формула считала
       бы от цвета, которого на экране нет. */
    discoNestInk: 45,
    discoNestA: 0.72,
    discoNest: '',
  ...
  /* Размер парящих значков — переменной CSS, а не числом прямо в
     правилах. Причина та же, что у кегля полей формы: он зависит от
     масштаба страницы, а масштаб меняется без перезагрузки, тогда как
     подставленное шаблоном значение после сборки строки уже не
     изменить. Начальное значение — заводское, настоящее ставит
     jumpVars (раздел 6).

     Переменная одна на всех, кто висит поверх страницы: прыгалки
     справа, стрелка стыковки у левого края и гнездо у полосы
     прозрачности. Все трое — цели для пальца или ориентиры для него, и
     разъехаться в размере им нельзя: стыковка тем и делается, что
     стрелку подводят к гнезду, а расстояние между ними читается только
     вместе с их размером.

     Запасное значение в var() не для красоты: до первого вызова
     jumpVars переменной на документе нет вовсе, и без него кнопка
     осталась бы нулевой ширины.

     Ореолы считаются умножением на долю: при значке в jumpSize выходят
     ровно те радиусы, что заданы числами, а при выросшем значке ореол
     растёт вместе со значком. Пара одна на всю тройку — прыгалки,
     стрелку стыковки и метку у полосы прозрачности, — потому что и
     тон у них теперь общий. Отдельно стоит только свечение гнезда в
     режиме стыковки: там значок не отделяется от текста, а зовёт к
     себе. */
  var JUMP_VAR = 'var(--lm-jump, ' + CFG.jumpSize + 'px)';
  var jumpScale = function (px) {
    return 'calc(' + JUMP_VAR + ' * ' + (px / CFG.jumpSize).toFixed(5) + ')';
  };
  var JUMP_HALO1 = jumpScale(CFG.jumpHaloIn);
  var JUMP_HALO2 = jumpScale(CFG.jumpHaloOut);
  /* Свечение гнезда в режиме стыковки — та же затея, но шире: гнездо в
     этом режиме плотное и должно светиться, а не просто отделяться. */
  var DSC_GLOW1 = jumpScale(2);
  var DSC_GLOW2 = jumpScale(5);

  /* Заливка прыгалок. Два вызова на два набора тонов, чтобы число
     jumpInk стояло в настройках один раз.

     Почему в тёмной теме годится тот же приём с прозрачностью, что и в
     светлой, хотя страница там перевёрнута целиком. Полупрозрачная
     заливка сперва смешивается с тем, что под ней, и уже смесь уходит
     под общий переворот: inv(a·C + (1−a)·B) = a·inv(C) + (1−a)·inv(B).
     То есть указать inv(C) с той же непрозрачностью — ровно то же
     самое, что нарисовать C поверх перевёрнутой подложки. Никакой
     поправки на прозрачность не нужно, достаточно отразить тон.

     dark=true даёт цвет ДЛЯ ПРАВИЛА тёмной темы: он записан до
     переворота, а на экране окажется тоном (255 − jumpInk). */
  function jumpInkCss(dark) {
    var v = dark ? inv(inv(CFG.jumpInk)) : CFG.jumpInk;
    /* Отражение и переворот гасят друг друга: на экране в тёмной теме
       получается 255 − jumpInk, в правило же идёт сам jumpInk. Пишем
       обе операции явно, а не сокращаем, — иначе следующему читателю
       (и себе через месяц) непонятно, почему у двух тем одно число. */
    return 'rgba(' + v + ',' + v + ',' + v + ',' + CFG.jumpInkA + ')';
  }
  ...
  function darkRules() {
    ...
    /* Заливка значков задана фигурам атрибутом, поэтому перекрываем
       ...
       Все три парящих значка — прыгалки, стрелка стыковки и метка у
       полосы прозрачности — красятся одним тоном: в светлой теме они
       уже пара, и в тёмной обязаны остаться парой. Селектор тяжелее
       светлого (лишний класс при html) И стоит ниже по файлу —
       darkRules вставляется в самый конец таблицы, так что спор
       решается одинаково и по весу, и по порядку. */
    out.push(D + '#lm-nav button svg polygon,\n' +
             D + '.lm-dsc_nest svg polygon,\n' +
             D + '#lm-dsc_arrow svg polygon {\n' +
             '  fill: ' + jumpInkCss(true) + ' !important; }');
    /* Гнездо в режиме стыковки — исключение из этой тройки: оно цель,
       а не указатель, и тон у него свой тёмный.

       Правило нужно отдельным, и вот почему. Светлое правило гнезда
       (html.lm-sconnect .lm-dsc_nest svg polygon) весит ровно столько
       же, сколько строка выше, а стоит ВЫШЕ неё по файлу — значит без
       этой строки в тёмной теме гнездо получало бы общий тон и
       переставало отличаться от стрелки, которую в него вводят. Так
       оно и было раньше, просто разница между двумя серыми была не
       так заметна. Здесь два класса при html против одного — тяжелее,
       и ниже по файлу. */
    out.push('html.lm-dark.lm-sconnect .lm-dsc_nest svg polygon {\n' +
             '  fill: ' + discoNestColor(true) + ' !important; }');
    /* Белый ореол под значками после переворота становится чёрным и на
       ...
       Гвоздик навштуки стоит отдельно: он лежит В СТРОКЕ, вместе с
       текстом, и вместе с текстом же меняется от масштаба —
       привязывать его к размеру парящих значков было бы ошибкой.
       Пиксель и два у него так и остаются пикселем и двумя. */
    out.push(D + '.lm-navthing_pin svg {\n' +
             '  filter: drop-shadow(0 0 1px rgba(0,0,0,.95))\n' +
             '          drop-shadow(0 0 2px rgba(0,0,0,.8)) !important; }');
    /* У парящих значков ореол считается от размера, а он зависит от
       масштаба страницы: при выросшем значке ореол растёт вместе с
       ним, иначе на увеличенной кнопке он становится ниткой. Радиусы
       общие на всю тройку — она и по тону общая.
       Гнездо в режиме стыковки эта строка не задевает: у него вместо
       ореола свечение, и оно задано отдельным правилом выше по
       файлу... которое ниже по весу. Поэтому селектор стыковки в
       светлой части весит на класс больше — проверять надо там. */
    out.push(D + '#lm-nav button svg,\n' +
             D + '.lm-dsc_nest svg,\n' +
             D + '#lm-dsc_arrow svg {\n' +
             '  filter: drop-shadow(0 0 ' + JUMP_HALO1 + ' rgba(0,0,0,.95))\n' +
             '          drop-shadow(0 0 ' + JUMP_HALO2 + ' rgba(0,0,0,.8)) !important; }');
    /* Свечение гнезда в режиме стыковки. Без этой строки его съедало бы
       правило ореола выше: там три класса при html против двух у
       светлого правила стыковки, и в тёмной теме гнездо теряло бы
       свечение — то самое, по которому и понятно, куда вести стрелку.
       Здесь классов три, и строка стоит ниже. Цвет свечения переворот
       переживает как есть: он задан настройкой, и угадывать за
       человека, какой оттенок он имел в виду, мы не будем. */
    out.push('html.lm-dark.lm-sconnect .lm-dsc_nest svg {\n' +
    ...
    out.push(D + '.b-logo img,\n' + D + '.b-logo_subsite img {\n' +
             '  opacity: .7 !important; }');
    /* Вектор картинкой не считается. Из общей инверсии исключены img,
       video, canvas и прочее — узел со встроенным svg под это правило
       не подпадает и уходит в переворот вместе со страницей: маска
       синеет, звезда зеленеет. Возвращаем цвета тем же приёмом, что у
       короны президента и у эмодзи, и приглушаем ровно так же, как
       растр строкой выше, — чтобы при переключении настройки
       logoVector в тёмной теме ничего не менялось на вид. */
    out.push(D + '.lm-logo_vec {\n' +
             '  filter: invert(1) hue-rotate(180deg) !important;\n' +
             '  opacity: .7 !important; }');
    /* У подсайтовой маски прозрачность своя и в светлой теме (.85),
       и множить её на .7 нельзя — на тёмном она пропадала бы. Правило
       тяжелее предыдущего (два класса против одного) И ниже по файлу. */
    out.push(D + '.lm-logo_vec.lm-logo_vec__sub {\n' +
             '  opacity: .6 !important; }');
    /* Герб в строке навштуки — та же история и то же лекарство. Пока
       он был картинкой, цвета ему возвращало общее правило для img;
       вектор под него не подпадает. Приглушать не надо: он и так
       мелкий, а на тёмном фоне цветной шарик в двадцать один пиксель
       не кричит. */
    out.push(D + '.lm-navthing_icon__vec {\n' +
             '  filter: invert(1) hue-rotate(180deg) !important; }');
  ...
  /* Здесь был и ALT_JUMP — «231 − 45» для парящих значков. Убран
     вместе со своим правилом: значки перестали быть светло-серыми и
     тонуть в притемнённом фоне им больше нечем, а висят они над
     светлыми карточками, а не над страницей. Разбор — у самого
     правила, ближе к концу этого раздела. */
...
html {
  ...
  /* Прыгалки: сторона значка, просвет между значками и отступ от
     правого края. Все три зависят от масштаба страницы и по той же
     причине вынесены в переменные; здесь стоят заводские числа — те,
     что получаются при стопроцентном масштабе. Настоящие ставит
     jumpVars. */
  --lm-jump: ${CFG.jumpSize}px;
  --lm-jump-gap: ${CFG.jumpGap}px;
  --lm-jump-right: ${CFG.jumpRight}px;
...
/* ---- Логотип фигурами вместо картинки ----
   ...
   vectorLogo. Ссылка со всем, что на ней висит, остаётся лепровской;
   выключить CFG.logoVector — и картинка вернётся сама, потому что она
   никуда не девалась.
   display: none, а не прозрачность: невидимая картинка продолжала бы
   занимать место в строке, и логотип уехал бы на её ширину вправо. */
.b-logo img.lm-logo_raster,
.b-logo_subsite img.lm-logo_raster { display: none !important; }

/* Высота, а не ширина: у растра стоит потолок 40 на 40, и при
   пропорциях 50 на 47 он доходит до сорока именно по ширине, а по
   высоте занимает 38. У нашего рисунка стороны почти равны, и задай
   мы ширину — значок оказался бы на пару пикселей выше растра и
   сдвинул бы строку шапки. Задаём высоту, ширину считает браузер. */
.lm-logo_vec {
  display: inline-block !important;
  position: relative !important; top: -1px !important;
...
/* Кнопка — только площадь нажатия: ни фона, ни рамки, весь вид несёт
   ...
   Сторона — переменной: она считается от масштаба страницы, чтобы
   кнопка была одного размера НА ЭКРАНЕ, а не в записи стилей (см.
   jumpVars). Просвет и отступ от края в правиле выше — по той же
   причине и тем же множителем. */
#lm-nav button {
  pointer-events: auto !important;
  width: ${JUMP_VAR} !important; height: ${JUMP_VAR} !important;
  padding: 0 !important; margin: 0 !important;
  -webkit-appearance: none !important; appearance: none !important;
  background: none !important; border: 0 !important;
  border-radius: 0 !important; box-shadow: none !important;
...
/* Заливка. У лепры значки светло-серые: на десктопе они лежат в пустом
   поле справа, там это уместно. У нас они висят над текстом поста, и
   светлое на светлом теряется — берём тон заметно темнее, а
   читаемость того, что под значком, возвращаем прозрачностью.

   Правилом, а не атрибутом фигуры: заливку значку ставит jumpIcon при
   сборке, и менять её пришлось бы перерисовыванием. Правило перебивает
   атрибут при любом весе.

   Только polygon: глаза маски — отдельные ellipse, и они остаются
   красными и непрозрачными. Это единственное цветное пятно во всём
   наборе, гасить его незачем.

   Тот же тон стоит у стрелки стыковки и у метки полосы прозрачности:
   значки одного рода и одной работы, и разъезжаться им незачем. Своё
   есть только у гнезда в режиме стыковки — оно цель, а не указатель
   (см. CFG.discoNestInk). */
#lm-nav button svg polygon { fill: ${jumpInkCss(false)} !important; }
${CFG.jumpHalo ? `#lm-nav button svg {
  filter: drop-shadow(0 0 ${JUMP_HALO1} rgba(255,255,255,.95))
          drop-shadow(0 0 ${JUMP_HALO2} rgba(255,255,255,.8)) !important; }` : ''}
...
.lm-navthing_icon img {
  ...
  max-width: none !important; max-height: none !important;
  margin: 0 !important; }
/* Вектор вписывается в гнездо целиком: обрезать в нём нечего, полей
   у него нет. Всё окошко выше — про картинку лепры, у которой рисунок
   занимает 39 из 65 пикселей ширины файла. */
.lm-navthing_icon__vec svg {
  display: block !important;
  width: 100% !important; height: 100% !important; }
.lm-navthing_arrow {
...
/* Метка у полосы прозрачности. Один и тот же узел в двух видах.
   Без scroll-connectа это стрелка вниз, по которой разворачивают пост
   тапом. Такая же цель для пальца поверх текста, как прыгалки справа,
   — значит и выглядеть должна так же: тот же тон, та же прозрачность,
   тот же ореол. Раньше здесь стоял светло-серый цвет лепры, и после
   потемнения прыгалок эти два значка перестали быть парой.
   С scroll-connectом она становится гнездом: тот же силуэт, но заливка
   тёмная и вокруг свечение — в него предстоит вводить стрелку
   прокруткой. Прозрачности там меньше: гнездо обязано читаться целью
   (см. CFG.discoNestInk). */
.lm-dsc_nest {
  position: absolute !important; z-index: 4 !important;
  left: 0 !important;
  /* Сторона — той же переменной, что у прыгалок: значок стоит поверх
     страницы и служит целью для пальца, а на экране должен занимать
     одно и то же место при любом масштабе. Отсчёт середины гнезда по
     вертикали ставит скрипт (discoNest), но половину стороны он
     отдаёт вычитать самому CSS — иначе при изменении масштаба гнездо
     съехало бы с полосы прозрачности на половину разницы. */
  width: ${JUMP_VAR} !important; height: ${JUMP_VAR} !important;
  padding: 0 !important; margin: 0 !important;
  -webkit-appearance: none !important; appearance: none !important;
  background: none !important; border: 0 !important;
...
.lm-dsc_nest svg polygon { fill: ${jumpInkCss(false)} !important; }
...
/* Стрелка. Одна на всю страницу, стоит у левого края по середине
   ...
   делает и делать не должен, поэтому касания сквозь неё проходят.
   Отрицательное поле — половина стороны, тем же calc: только так
   стрелка остаётся ровно по середине экрана при выросшем значке.
   Деление на минус два в calc законно и даёт длину со знаком. */
#lm-dsc_arrow {
  position: fixed !important; top: 50% !important; left: 0 !important;
  width: ${JUMP_VAR} !important; height: ${JUMP_VAR} !important;
  margin-top: calc(${JUMP_VAR} / -2) !important;
...
#lm-dsc_arrow svg {
  ...
  filter: drop-shadow(0 0 ${JUMP_HALO1} rgba(255,255,255,.95))
          drop-shadow(0 0 ${JUMP_HALO2} rgba(255,255,255,.8)) !important; }
#lm-dsc_arrow svg polygon { fill: ${jumpInkCss(false)} !important; }
...
/* Дорожка значков. Стрелка и гнездо в НАСТОЯЩЕМ размере — и это не
   украшательство. Допуск тут показан расстоянием между двумя
   предметами, а расстояние читается только вместе с их размером: те же
   семнадцать пикселей между значками в сорок и между значками в
   двадцать четыре выглядят совершенно по-разному, и уменьшенная
   картинка врала про то, насколько точно надо попасть. Раз врёт — она
   бесполезна, ради неё раздел и заведён.

   Отсюда и переменная вместо числа. С тех пор как настоящий размер
   зависит от масштаба страницы, число ${CFG.jumpSize} перестало быть
   настоящим везде, кроме стопроцентного масштаба: попап уезжает вместе
   со страницей, а значок на странице — нет, и картинка снова начала бы
   врать, причём тем сильнее, чем дальше масштаб от ста. С переменной
   пара в попапе занимает на экране ровно столько же, сколько занимает
   настоящая пара при стыковке.
...
.lm-set_dockpic {
  flex: 0 0 auto !important;
  width: calc(${JUMP_VAR} + 16px) !important;
  ...
  height: calc(${JUMP_VAR} + 30px) !important;
  display: flex !important; flex-direction: column !important;
  align-items: center !important; justify-content: center !important; }
...
/* Особого тона для парящих значков в обратном наборе больше нет, и это
   не упущение. Подбирали его потому, что лепровский светло-серый
   терялся на притемнённой странице; теперь у прыгалок, стрелки
   стыковки и метки у полосы прозрачности свой тёмный полупрозрачный
   тон, а висят все трое над КАРТОЧКАМИ, а карточки в обратном наборе
   как раз светлые. То есть под значком ровно то же, что и в заводском
   наборе, и второй тон был бы разницей без причины.

   Оставить их здесь было нельзя даже при желании: селектор обратного
   набора тяжелее общего (лишние два класса при html), и он молча
   перекрыл бы и цвет, и прозрачность — а заметно это стало бы только
   у тех, кто включил «Фон». */
...
`;
  ...
  /* Вектор вместо растра логотипа.

     Разметку лепры не трогаем: картинку НЕ удаляем и не подменяем ей
     адрес, а гасим правилом по классу — и кладём свой рисунок рядом,
     внутрь той же ссылки. Ссылка со всем, что на ней висит, остаётся
     нетронутой; выключить настройку — и растр вернётся на место сам,
     потому что он никуда и не девался.

     Класс, а не инлайновый display: у лепры на логотипе бывает свой
     инлайновый стиль (на подсайтах), и переписывать его значило бы
     потерять то, что она туда положила.

     На подсайте логотипов два: свой у подсайта и маска лепры рядом с
     ним. Векторим оба — маска там та же самая, только помельче. */
  function vectorLogo(a, cls) {
    if (!CFG.logoVector || !a || a.dataset.lmVec) return;
    a.dataset.lmVec = '1';
    var box = document.createElement('span');
    box.className = 'lm-logo_vec' + (cls ? ' ' + cls : '');
    box.setAttribute('aria-hidden', 'true');
    box.innerHTML = LOGO_SVG;
    var img = a.querySelector('img');
    if (img) {
      img.classList.add('lm-logo_raster');
      a.insertBefore(box, img);
    } else {
      a.appendChild(box);
    }
  }
  ...
  function relayoutHeader() {
    ...
    /* Векторим логотипы. Своим — только тот, что НЕ подсайтовый: на
       подсайте лепра подменяет картинку .b-logo своим оформлением, и
       рисовать поверх него маску значило бы стереть подсайту лицо.
       Выбираем по классу, а не по id: js-logo у лепры продублирован на
       обоих логотипах.
       Ищем по документу, а не внутри .l-header, и это не мелочь:
       логотип к этому моменту мог уже уехать в липкую плашку
       (buildTopbar забирает .b-logo вместе с приветствием), и селектор
  ...
  function openSettings() {
    ...
    var mkTri = function (cls, fill, glow) {
      ...
      var svg = svgNode('svg', {
        viewBox: '0 0 40 40',
        width: String(jumpNow()), height: String(jumpNow()),
        xmlns: SVG_NS, 'aria-hidden': 'true'
      });
      ...
      /* Свечение — по текущему размеру, как у настоящего гнезда: на
         сорока это те же два и пять. Числами, а не переменной, потому
         что тут инлайновый стиль; попап собирается заново при каждом
         открытии, так что застрять на старом масштабе оно не успеет. */
      if (glow) {
        var g1 = (2 * jumpNow() / CFG.jumpSize).toFixed(2);
        var g2 = (5 * jumpNow() / CFG.jumpSize).toFixed(2);
        svg.style.filter =
          'drop-shadow(0 0 ' + g1 + 'px ' + CFG.discoGlow + ') ' +
          'drop-shadow(0 0 ' + g2 + 'px ' + CFG.discoGlow + ')';
      }
      b.appendChild(svg);
      return b;
    };
    var dockArrow = mkTri('lm-set_arrow', jumpInkCss(false), false);
    var dockNest = mkTri('lm-set_nest', discoNestColor(), true);
    ...
    var dockDraw = function () {
      ...
      /* Расстояние между ЦЕНТРАМИ значков, а не между их коробками.
         ...
         Вычитается рост значка: расстояние между центрами равно
         «отбивка плюс рост», и чтобы центры разошлись ровно на
         discoDock, отбивке остаётся разница. Рост берём переменной, а
         не числом: значки в дорожке нарисованы ею же, и посчитанная
         разница устарела бы при первом изменении масштаба — пара
         разъехалась бы ровно на него.
      ...
      dockNest.style.setProperty('margin-top',
        'calc(' + CFG.discoDock + 'px - ' + JUMP_VAR + ')', 'important');
    };
  ...
  /* ---- Герб фигурами ----
     ...
     Оговорка, без которой числа ниже читаются неверно. Оригинал
     (empire_blogs.png, рисунок 39×38 внутри файла 65×58) — не рисунок,
     а фотоколлаж: шар, собранный из разноцветной крошки. Формы там
     нет, обводить нечего. Это стилизация — но устройство и доли цветов
     сняты с образца, а не выдуманы:

       радиус шара 17 из 39 (по спаду альфы);
       в середине БЕЛАЯ ПЯТИЛУЧЕВАЯ РОЗЕТКА — её видно только при
       увеличении раз в десять, и первый заход, построенный по
       усреднённым цветам колец, её проглядел: вышел ровный «зонтик»,
       похожий на круговую диаграмму;
       крошка идёт кольцами вокруг розетки и густеет к краю;
       по кромке тёмная кайма — кольцо 10–17 темнее кольца 5–10
       примерно на два десятка по каждому каналу;
       разбор по тону: 166 пикселей красно-оранжевых, 139
       жёлто-охристых, 16 малиновых, 11 синих, 4 зелёных. Шар тёплый,
       холодное в нём редкими крапинами — столько же их и в мешке
       цветов ниже.

     Зачем вообще. Чёткость тут не главное: на двадцати одном пикселе
     разница с растром невелика. Главное, что адрес картинки берётся со
     страницы или из памяти, и на внутренних страницах, где правой
     колонки нет, гнездо оставалось пустым. Свой значок есть всегда.
     Заодно уходит окошко над картинкой со своими четырьмя замерами
     файла: они разъехались бы, подмени лепра герб.

     Генератором, а не готовой строкой: строка вышла бы на двенадцать
     килобайт нечитаемых координат, а тут полсотни строк, в которых
     видно устройство и которые можно покрутить. */
  var CREST_PAL = {
    red: '#c33428', red2: '#d9493b', red3: '#eb594f', pink: '#ff6987',
    crim: '#b92039', amber: '#e7b351', gold: '#d2b53e', ochre: '#a87814',
    rust: '#aa513a', brown: '#954430', olive: '#948e4d',
    blue: '#5289b5', green: '#74a683'
  };

  /* Мешок цветов в тех же долях, что на образце: тёплое подряд,
     холодное поодиночке. Порядок перемешан заранее и записан как есть. */
  var CREST_BAG = [
    'red2', 'amber', 'red', 'gold', 'rust', 'red3', 'ochre', 'amber',
    'red2', 'olive', 'crim', 'gold', 'red', 'amber', 'blue', 'red3',
    'brown', 'gold', 'red2', 'ochre', 'amber', 'red', 'rust', 'gold',
    'red3', 'amber', 'pink', 'red2', 'olive', 'gold', 'red', 'amber',
    'green', 'red2', 'ochre', 'gold', 'rust', 'red3', 'amber', 'crim',
    'red', 'gold', 'brown', 'amber', 'red2', 'blue', 'gold', 'red3',
    'olive', 'amber', 'red', 'ochre', 'gold', 'red2', 'rust', 'amber'
  ];
  ...
  function crestSvg() {
    ...
    var C = 20, R = 19;
    /* Детерминированный шум. Math.random дал бы при каждой загрузке свой
       значок — а он должен быть один и тот же, иначе человек видит, как
       герб «моргает» при переходах, и сверять правки не с чем.
       Обычный линейный конгруэнтный, зерно зашито. */
    var seed = 20260825;
    var rnd = function () {
    ...
    var p = function (a, r) {
      return (C + r * Math.cos(a * Math.PI / 180)).toFixed(2) + ' ' +
             (C + r * Math.sin(a * Math.PI / 180)).toFixed(2);
    };
    /* Крошинка — кусок кольца: сектор в несколько градусов между двумя
       радиусами. Углы и радиусы рваные, просветы остаются: у образца
       между кусочками просвечивает белое. */
    var chip = function (a0, a1, r0, r1, fill) {
      return '<path d="M ' + p(a0, r0) + ' L ' + p(a0, r1) +
             ' A ' + r1.toFixed(2) + ' ' + r1.toFixed(2) + ' 0 0 1 ' + p(a1, r1) +
             ' L ' + p(a1, r0) +
             ' A ' + r0.toFixed(2) + ' ' + r0.toFixed(2) + ' 0 0 0 ' + p(a0, r0) +
             ' Z" fill="' + CREST_PAL[fill] + '"/>';
    };

    /* Четыре кольца, к краю плотнее. Начальный угол у каждого свой, и
       ширина кусочка гуляет вдвое: ровные ряды читаются сеткой, а
       нужна крошка. Доли кусочков по кольцам — от общего числа. */
    var n = Math.max(24, CFG.crestChips | 0);
    var rings = [[0.28, 0.52, 0.18], [0.44, 0.7, 0.23],
                 [0.63, 0.88, 0.27], [0.8, 1.0, 0.32]];
    var chips = [], bag = 0;
    rings.forEach(function (ring) {
      var cnt = Math.max(6, Math.round(n * ring[2]));
      var a = rnd() * 360, step = 360 / cnt;
      for (var i = 0; i < cnt; i++) {
        chips.push(chip(a, a + step * (0.7 + rnd() * 0.55),
                        R * (ring[0] + rnd() * 0.06),
                        R * (ring[1] - rnd() * 0.09),
                        CREST_BAG[bag++ % CREST_BAG.length]));
        a += step;
      }
    });

    /* Розетка. Пять лучей, один смотрит вверх — как на образце.

       Не звезда, а ШТУРВАЛ: концы лучей на образце не острые, а сильно
       скруглённые, при той же длине. Первый заход рисовал многоугольник
       с острыми вершинами, и это была единственная заметная неправда в
       значке.
       Рисуем спицами: отрезок из середины наружу с круглым концом
       (stroke-linecap: round) — он и даёт тупой скруглённый луч ровно
       той толщины, какой задан. Втулка в середине нужна, чтобы спицы
       не сходились в точку и центр остался ровным белым пятном. */
    var spokes = [];
    for (var i = 0; i < 5; i++) {
      var a = -90 + i * 72, r = R * 0.6;
      spokes.push('<path d="M ' + C + ' ' + C + ' L ' +
                  (C + r * Math.cos(a * Math.PI / 180)).toFixed(2) + ' ' +
                  (C + r * Math.sin(a * Math.PI / 180)).toFixed(2) +
                  '" stroke="#fbf7f2" stroke-width="' + (R * 0.26).toFixed(2) +
    ...
    crestCache =
      ...
      '<defs>' +
      /* Затемнение у самой кромки — оно и делает из круга шар. */
      '<radialGradient id="lmCrestEdge" cx="0.5" cy="0.5" r="0.5">' +
      '<stop offset="0.7" stop-color="#000000" stop-opacity="0"/>' +
      '<stop offset="1" stop-color="#3a2a1c" stop-opacity="0.45"/>' +
      '</radialGradient>' +
      /* Крошинки считаются по дугам и на стыках дают заусенцы за
         кромкой — режем всё ровным кругом. */
      '<clipPath id="lmCrestClip">' +
      '<circle cx="' + C + '" cy="' + C + '" r="' + R + '"/></clipPath>' +
      '</defs>' +
      '<g clip-path="url(#lmCrestClip)">' +
      '<circle cx="' + C + '" cy="' + C + '" r="' + R + '" fill="#fbf7f2"/>' +
      chips.join('') +
      spokes.join('') +
      '<circle cx="' + C + '" cy="' + C + '" r="' + (R * 0.2).toFixed(2) +
      '" fill="#fbf7f2"/>' +
      '<circle cx="' + C + '" cy="' + C + '" r="' + R +
      '" fill="url(#lmCrestEdge)"/>' +
      '</g></svg>';
    return crestCache;
  }
  ...
  /* ============================================================
     ЛОГОТИП ЛЕПРЫ ВЕКТОРОМ
     ============================================================

     Растр лепры — lepro_logo_mask.png, 50 на 47. В шапке он стоит с
     потолком 40 на 40, то есть почти без увеличения, и на экране с
     удвоенной плотностью точек показывается мылом. Здесь тот же
     рисунок фигурами.

     Числа сняты замером с крупного варианта того же логотипа
     (512 на 512), а не подобраны на глаз, и сверялись наложением:

       звезда НАРИСОВАНА ОТ РУКИ — шаг между лучами гуляет от 30 до 42
       градусов, радиус кончика от 198 до 211. Правильная десятиконечная
       рядом с образцом выглядит штампом, поэтому каждый луч стоит
       там, где стоял; два луча целиком закрыты маской и в замер не
       попали — они по середине между соседями;
       впадины на 112: первый заход стоял на 122, и лучи выходили
       заметно толще образца;
       контур маски снят по строкам через каждые двенадцать и проведён
       гладкой кривой ровно по замеру;
       бровь — РОМБ, а не полоса: на строке 220 в замере два отдельных
       куска, на 224 уже слитный, дальше сужается к острию на 239;
       заклёпки найдены разбором картинки (тень темнее фона маски, но
       не красная и не чёрная) — все девятнадцать вышли парами,
       симметричными относительно середины.

     viewBox обрезан по габаритам звезды, а не оставлен квадратным.
     Причина не в аккуратности: файл лепры обрезан по содержимому,
     полей в нём нет вовсе, и выводится он с потолком 40. Квадратный
     кадр со свободными полями дал бы значок ощутимо мельче растра при
     том же потолке — на снимке в настоящем размере это сразу видно,
     глаза переставали читаться.

     Чего здесь НЕТ: светло-серой тени-подложки под звездой. На сорока
     пикселях она превращается в грязь по контуру.

     Готовой строкой, а не сборкой из чисел: собирать этот рисунок
     кодом — это двести строк ради того, что не меняется. */
  var LOGO_SVG =
    '<svg viewBox="10.94 9.76 78.5 81.46" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="lmMaskG" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#f1e0c9"/><stop offset="1" stop-color="#dcc5a4"/>' +
    '</linearGradient><radialGradient id="lmEyeG" cx="0.4" cy="0.34" r="0.8">' +
    '<stop offset="0" stop-color="#4a423b"/><stop offset="1" stop-color="#120f0c"/>' +
    '</radialGradient><radialGradient id="lmIrisG" cx="0.5" cy="0.5" r="0.5">' +
    '<stop offset="0" stop-color="#ff5a3a"/>' +
    '<stop offset="0.45" stop-color="#e21206"/>' +
    '<stop offset="1" stop-color="#8c0a04"/></radialGradient></defs>' +
    '<polygon points="67.24,63.47 56.82,70.79 43.37,70.85 32.43,63.05 28.12,50.38 32.08,37.45 43.09,29.24 57.97,29.62 68.5,38.32 71.86,50.92" fill="#141414"/>' +
    '<polygon points="71.86,50.92 86.27,64.51 67.24,63.47" fill="#141414"/>' +
    '<polygon points="67.24,63.47 74.11,83.42 56.82,70.79" fill="#dc4a3c"/>' +
    '<polygon points="56.82,70.79 50.36,90.82 43.37,70.85" fill="#141414"/>' +
    '<polygon points="43.37,70.85 25.89,83.42 32.43,63.05" fill="#dc4a3c"/>' +
    '<polygon points="32.43,63.05 11.96,63.1 28.12,50.38" fill="#141414"/>' +
    '<polygon points="28.12,50.38 11.34,38.18 32.08,37.45" fill="#dc4a3c"/>' +
    '<polygon points="32.08,37.45 25.79,17.87 43.09,29.24" fill="#141414"/>' +
    '<polygon points="43.09,29.24 50.14,10.16 57.97,29.62" fill="#dc4a3c"/>' +
    '<polygon points="57.97,29.62 76.13,21.49 68.5,38.32" fill="#141414"/>' +
    '<polygon points="68.5,38.32 89.04,38.07 71.86,50.92" fill="#dc4a3c"/>' +
    '<ellipse cx="28.91" cy="49.61" rx="1.76" ry="2.73" fill="#dcc6a8"/>' +
    '<ellipse cx="71.09" cy="49.61" rx="1.76" ry="2.73" fill="#dcc6a8"/>' +
    '<path d="M 50 25.39 C 52.73 25.39 56.06 26.3 58.01 26.95 C 59.96 27.6 60.68 28.52 61.72 29.3 C 62.76 30.08 63.58 30.86 64.26 31.64 C 64.94 32.42 65.33 33.2 65.82 33.98 C 66.31 34.76 66.83 35.55 67.19 36.33 C 67.55 37.11 67.74 37.89 67.97 38.67 C 68.2 39.45 68.39 40.24 68.55 41.02 C 68.71 41.8 68.82 42.58 68.95 43.36 C 69.08 44.14 69.21 44.92 69.34 45.7 C 69.47 46.48 69.63 47.27 69.73 48.05 C 69.83 48.83 69.98 49.61 69.92 50.39 C 69.86 51.17 69.47 51.95 69.34 52.73 C 69.21 53.51 69.27 54.3 69.14 55.08 C 69.01 55.86 68.75 56.64 68.55 57.42 C 68.35 58.2 68.2 58.99 67.97 59.77 C 67.74 60.55 67.52 61.33 67.19 62.11 C 66.86 62.89 66.38 63.67 66.02 64.45 C 65.66 65.23 65.46 66.02 65.04 66.8 C 64.62 67.58 64.07 68.36 63.48 69.14 C 62.89 69.92 62.2 70.7 61.52 71.48 C 60.84 72.26 61.3 72.92 59.38 73.83 C 57.46 74.74 53.13 76.95 50 76.95 C 46.88 76.95 42.58 74.74 40.63 73.83 C 38.68 72.92 38.97 72.26 38.28 71.48 C 37.59 70.7 37.07 69.92 36.52 69.14 C 35.97 68.36 35.41 67.58 34.96 66.8 C 34.51 66.02 34.18 65.23 33.79 64.45 C 33.4 63.67 32.95 62.89 32.62 62.11 C 32.29 61.33 32.07 60.55 31.84 59.77 C 31.61 58.99 31.41 58.2 31.25 57.42 C 31.09 56.64 30.96 55.86 30.86 55.08 C 30.76 54.3 30.82 53.51 30.66 52.73 C 30.5 51.95 29.98 51.17 29.88 50.39 C 29.78 49.61 29.98 48.83 30.08 48.05 C 30.18 47.27 30.34 46.48 30.47 45.7 C 30.6 44.92 30.7 44.14 30.86 43.36 C 31.02 42.58 31.29 41.8 31.45 41.02 C 31.61 40.24 31.61 39.45 31.84 38.67 C 32.07 37.89 32.45 37.11 32.81 36.33 C 33.17 35.55 33.49 34.76 33.98 33.98 C 34.47 33.2 35.05 32.42 35.74 31.64 C 36.43 30.86 37.11 30.08 38.09 29.3 C 39.07 28.52 39.62 27.6 41.6 26.95 C 43.59 26.3 47.27 25.39 50 25.39 Z" fill="url(#lmMaskG)"/>' +
    '<circle cx="40.82" cy="31.05" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="40.82" cy="30.76" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="59.96" cy="31.05" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="59.96" cy="30.76" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="40.43" cy="35.55" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="40.43" cy="35.25" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="60.35" cy="35.35" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="60.35" cy="35.06" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="40.23" cy="40.43" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="40.23" cy="40.14" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="60.55" cy="40.43" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="60.55" cy="40.14" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="46.68" cy="40.82" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="46.68" cy="40.53" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="54.1" cy="40.82" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="54.1" cy="40.53" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="37.11" cy="57.81" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="37.11" cy="57.52" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="63.48" cy="57.81" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="63.48" cy="57.52" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="50.59" cy="59.18" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="50.59" cy="58.89" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="36.13" cy="62.3" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="36.13" cy="62.01" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="44.92" cy="62.3" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="44.92" cy="62.01" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="55.66" cy="62.3" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="55.66" cy="62.01" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="64.45" cy="62.3" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="64.45" cy="62.01" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="44.92" cy="66.6" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="44.92" cy="66.31" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="55.66" cy="66.6" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="55.66" cy="66.31" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="45.12" cy="71.68" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="45.12" cy="71.39" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="55.27" cy="71.68" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="55.27" cy="71.39" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="50" cy="28.52" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="50" cy="28.22" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="45.31" cy="29.49" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="45.31" cy="29.2" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="54.69" cy="29.49" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="54.69" cy="29.2" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="34.77" cy="48.05" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="34.77" cy="47.75" r="0.82" fill="#f2e6d6"/>' +
    '<circle cx="66.02" cy="48.05" r="1.17" fill="#dcc8ac"/>' +
    '<circle cx="66.02" cy="47.75" r="0.82" fill="#f2e6d6"/>' +
    '<path d="M 42.19 42.58 L 50.59 43.55 L 58.01 42.58 L 50.59 46.68 Z" fill="#dc4a3c"/>' +
    '<path d="M 45.12 56.25 L 38.28 63.48 L 40.43 66.02 Z" fill="#dc4a3c"/>' +
    '<path d="M 54.88 56.25 L 61.72 63.48 L 59.57 66.02 Z" fill="#dc4a3c"/>' +
    '<ellipse cx="40.63" cy="50.2" rx="5.66" ry="4.79" fill="url(#lmEyeG)"/>' +
    '<circle cx="41.5" cy="50.2" r="1.95" fill="url(#lmIrisG)"/>' +
    '<ellipse cx="60.45" cy="50.2" rx="5.66" ry="4.79" fill="url(#lmEyeG)"/>' +
    '<circle cx="59.86" cy="50.2" r="1.95" fill="url(#lmIrisG)"/></svg>';
  ...
  function buildNavthing() {
    ...
    if (CFG.crestVector) {
      /* Свой значок есть всегда — в отличие от картинки, которой на
         внутренних страницах может не быть ни на странице, ни в
         памяти, и гнездо оставалось пустым. Класс на гнезде отменяет
         окошко: вектор вписывается целиком, обрезать нечего. */
      icon.classList.add('lm-navthing_icon__vec');
      icon.innerHTML = crestSvg();
    } else {
      var crest = crestSrc();
      if (crest) {
  ...
  /* Размер прыгалок с поправкой на масштаб страницы.

     Зачем. Сорок пикселей CSS равны сорока пикселям экрана только при
     стопроцентном масштабе. Меню «Аа» в Safari масштаб меняет: при 85%
     значок доходит до глаза тридцатью четырьмя, при увеличенном —
     раздувается. Палец при этом не меняется, а в эти кнопки целятся
     вслепую, не глядя, — им положено быть одного размера всегда.

     Масштаб берём у visualViewport.scale — из того же источника и по
     той же причине, что кегль полей формы (formFontVar) и поля поиска
     (searchFontVar): другого источника нет, а меню «Аа» меняет именно
     его. Нет visualViewport — считаем масштаб единичным, хуже
     прежнего от этого не станет.

     Пределы нужны с обеих сторон: при масштабе 200% деление дало бы
     двадцать, при 50% — восемьдесят, и в обоих концах кнопка перестаёт
     быть кнопкой.

     Просвет и отступ считаются НЕ делением, а тем множителем, который
     на деле вышел у значка. Разница видна только на упоре в предел:
     там значок останавливается, а деление продолжало бы разгонять
     просветы, и столбик разъезжался бы вокруг стоящих кнопок.

     Пишем ТОЛЬКО при расхождении. Слушатель висит на
     visualViewport.resize, а это событие на iOS приходит не только от
     клавиатуры и щипка, но и от прячущейся адресной строки — то есть
     при обычной прокрутке. Масштаб между такими событиями почти
     никогда не меняется, а setProperty трогает раскладку. Та же
     оговорка и теми же словами стоит в formFontVar и syncTopbarVars. */
  var jumpScaleSeen = 1, jumpSizeSeen = 0;

  function jumpVars() {
    var k = 1;
    if (CFG.jumpZoom) {
      var vv = window.visualViewport;
      if (vv && vv.scale) k = vv.scale;
    }
    jumpScaleSeen = k;

    var px = Math.round(CFG.jumpSize / k);
    px = Math.min(CFG.jumpMax, Math.max(CFG.jumpMin, px));
    jumpSizeSeen = px;

    var f = px / CFG.jumpSize;
    var el = document.documentElement;
    ...
    put('--lm-jump', px);
    put('--lm-jump-gap', Math.round(CFG.jumpGap * f));
    put('--lm-jump-right', Math.round(CFG.jumpRight * f));
  }

  /* Текущая сторона парящих значков — для тех мест, где размер нужен
     скрипту, а не таблице стилей: гнездо стыковки скрипт ставит сам,
     отсчитывая его середину от полосы прозрачности.

     Запасное значение обязательно: гнёзда бывают построены раньше
     первого вызова jumpVars (полный проход зовёт jumpVars первым, но
     лёгкий проход и догрузка постов идут своим чередом), и ноль в
     половине стороны увёл бы гнездо вверх ровно на двадцать пикселей.
     Ошибка была бы тихой: гнездо на месте, просто не по центру полосы,
     а стыковка от этого мажет. */
  function jumpNow() { return jumpSizeSeen || CFG.jumpSize; }
  ...
  function watchZoom() {
    ...
    zoomWatched = true;
    /* Один слушатель на обе поправки: событие приходит то же самое, а
       второй обработчик на том же событии — лишняя работа в момент,
       когда её и так много (это же событие шлёт прячущаяся адресная
       строка при прокрутке). Обе функции пишут только при
       расхождении, поэтому холостой вызов ничего не стоит. */
    window.visualViewport.addEventListener('resize', function () {
  ...
  /* Цвет гнезда в режиме стыковки. Своя пара чисел, а не вычисление
     обратного к прыгалкам: обращать больше нечего (разбор в CFG рядом
     с discoNestInk). Прозрачность здесь меньше, чем у прыгалок,
     намеренно — гнездо должно читаться целью.

     dark=true отдаёт тон ДЛЯ ПРАВИЛА тёмной темы. Тот же приём и та же
     арифметика, что у прыгалок (jumpInkCss): чтобы на перевёрнутой
     странице значок выглядел отражённым тоном, в правило идёт
     отражение отражения, то есть само число. Прозрачность поправки не
     требует — полупрозрачная заливка смешивается с подложкой ДО общего
     переворота, и inv(a·C + (1−a)·B) = a·inv(C) + (1−a)·inv(B).
     Выходит, что на тёмной странице гнездо становится светлым пятном
     той же плотности: дырой в чёрном оно быть не должно, а заметно
     плотнее стрелки — должно.

     Готовая строка из CFG.discoNest переворот не переживает и в тёмной
     теме отдаётся как есть: подобрать пару вручную человек может, а
     угадать за него, что он имел в виду, мы не можем.

     Функция нужна и таблице стилей, и сборке значка в попапе, поэтому
     не строка, а вызов. */
  function discoNestColor(dark) {
    if (CFG.discoNest) return CFG.discoNest;
    var v = dark ? inv(inv(CFG.discoNestInk)) : CFG.discoNestInk;
    return 'rgba(' + v + ',' + v + ',' + v + ',' + CFG.discoNestA + ')';
  }
  ...
  function discoNest(post, dt) {
    ...
    /* Половину стороны вычитает САМ CSS, а не скрипт. Разница не в
       красоте: сторона теперь зависит от масштаба страницы, и число,
       посчитанное один раз при сборке, устарело бы при первом же
       повороте колёсика «Аа» — гнездо съехало бы с полосы
       прозрачности ровно на половину разницы, а стыковка мажет уже на
       нескольких пикселях. С calc середина пересчитывается браузером
       сама, как только меняется переменная, и переставлять гнёзда
       обходом страницы не нужно вовсе. */
    nest.style.top = 'calc(' + Math.round(bottom - CFG.discoFade / 2) +
                     'px - ' + JUMP_VAR + ' / 2)';
    return nest;
  }
  ...
  function discoScanComments() {
    ...
    place.forEach(function (q) {
      /* Тот же calc, что и в ленте, и по той же причине: середину
         пересчитывает браузер, когда меняется --lm-jump. */
      q.nest.style.top =
        'calc(' + Math.round(q.bottom - CFG.discoFade / 2) +
        'px - ' + JUMP_VAR + ' / 2)';
    });
  ...
  function report() {
    var L = ['Lepra Mobile v' + VERSION,
             ...
             /* Парящие значки: прыгалки справа, стрелка стыковки слева,
                гнездо у полосы прозрачности — все трое одного размера
                и от одной переменной. Печатаем не только размер, но и
                откуда он взялся: «поправка выключена» — это
                CFG.jumpZoom, «упёрлось в предел» — jumpMin/jumpMax, а
                масштаб, застрявший на единице при заведомо изменённом,
                — это visualViewport, который его не отдал. Рядом — что
                на деле стоит в переменной: спор между нашей записью и
                правилом виден только по факту. */
             'значки: ' + jumpSizeSeen + 'px' +
               (CFG.jumpZoom
                 ? ' при масштабе ' + Math.round(jumpScaleSeen * 100) / 100 +
                   (jumpSizeSeen === CFG.jumpMin ? ' (упёрлось в низ)' :
                    jumpSizeSeen === CFG.jumpMax ? ' (упёрлось в верх)' : '')
                 : ' (поправка выключена)') +
               ' | переменная: ' +
               (document.documentElement.style.getPropertyValue('--lm-jump') ||
                '(не ставили)'),
  ...
  function fullPass() {
    ...
    /* Размер парящих значков — ДО всего, что их строит. Прыгалки
       ...
       втором кадре. Гнёзда стыковки за размером следят сами, через
       calc в инлайновом top, но и им спокойнее, когда переменная уже
       стоит к моменту первого замера.
       Проход дешёвый (три сравнения строк) и идемпотентный, поэтому
       стоит и в полном проходе: масштаб меняют и без события, например
