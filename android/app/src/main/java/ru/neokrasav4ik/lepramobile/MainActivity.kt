...
class MainActivity : AppCompatActivity() {
    ...
    private inner class Client : WebViewClient() {
        ...
        override fun onReceivedSslError(
            ...
            /* ЧЕЙ ЭТО ХОСТ — РЕШАЕТСЯ ДО ВСЕГО ОСТАЛЬНОГО.

               Отчёт с Андроида 14 (1.0.6, Xiaomi): отказ пришёл на
               https://mc.yandex.ru/metrika/tag.js, сертификат выдан
               CN=localhost для CN=localhost. Это не лепра и не
               недосланный промежуточный — это подмена сертификата на
               самом телефоне: блокировщик рекламы с фильтрацией HTTPS,
               VPN или «экономия трафика». Своих корней такие ставят
               сколько угодно, а WebView с Андроида 7 доверяет только
               системным — отсюда отказ.

               Лепра при этом открылась: в журнале есть «нарисовали:
               https://leprosorium.ru/login/». А следом шла наша
               заглушка, потому что обработчик не смотрел на хост
               вовсе, — и человек получил «Сертификат лепры не принят»
               с советом чинить это на стороне лепры. Подпись верная по
               форме и ложная по сути, ровно то, что в этом проекте
               считается худшим родом вранья — и второй раз в этом же
               обработчике.

               Цена ошибки не в одном отчёте: ЛЮБОЙ блокировщик рекламы
               на ЛЮБОМ телефоне гасил приложение, стоило ему перехватить
               чужую аналитику. У SslError нет признака главного фрейма,
               поэтому различаем по хосту — тем же ours(), которым
               ходят ссылки. */
            val сбойный = error.url?.let { runCatching { Uri.parse(it) }.getOrNull() }
            val нашХост = сбойный != null && ours(сбойный)
            Diag.fact("отказ по сертификату",
                why + " | " + error.url + (if (нашХост) "" else " | ЧУЖОЙ ХОСТ"))
            Diag.fact("сертификат", who)
            Diag.log("отказ по сертификату: " + why + " на " + error.url)
            handler.cancel()
            if (!нашХост) {
                /* Отменили запрос и молчим. Аналитика, счётчики и прочее
                   чужое добро вправе не загрузиться: страница от этого не
                   ломается, а заглушка сломала бы её наверняка. */
                Diag.log("хост чужой (" + (сбойный?.host ?: "?") +
                    ") — страницу не трогаем")
                return
            }
            ...
            /* Объяснение переписано по факту с устройства. Прежнее валило
               вину на «центры, которые человек поставил себе сам», — а в
               отчёте оказался GlobalSign, обычный публичный центр. Подпись
               была верной по форме и ложной по сути, то есть ровно тем,
               что мы в этом проекте называем худшим родом вранья. */
            showPlain("Сертификат лепры не принят",
                why + ". Адрес: " + (error.url ?: HOME) + ". " + who +
                ". Часы телефона: " + clock + ". " +
                "Приложение проверяет сертификат по системному хранилищу Андроида, " +
                "а Chrome и Файрфокс носят своё собственное — поэтому у них лепра " +
                "открывается. Так бывает в двух случаях: сервер лепры не досылает " +
                "промежуточный сертификат (браузеры подкачивают его сами, приложения — " +
                "нет) либо корневой центр новее вашего Андроида. Чинится это на " +
                "стороне лепры, и мы уже разбираемся.")
        }
